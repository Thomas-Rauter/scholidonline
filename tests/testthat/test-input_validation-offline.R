...
testthat::test_that(
  ".scholidonline_as_scalar_character handles coercion and trimming",
  {
    testthat::expect_equal(
      .scholidonline_as_scalar_character(" abc ", "x"),
      "abc"
    )
    
    testthat::expect_equal(
      .scholidonline_as_scalar_character(factor("abc"), "x"),
      "abc"
    )
    
    testthat::expect_true(is.na(
      .scholidonline_as_scalar_character("   ", "x")
    ))
  }
)
...
testthat::test_that(
  ...
  {
    testthat::expect_error(
      .scholidonline_as_scalar_character(arg = "x"),
      "`x` is required"
    )
    
    testthat::expect_error(
      .scholidonline_as_scalar_character(NULL, "x"),
      "`x` must not be NULL"
    )
    
    testthat::expect_error(
      .scholidonline_as_scalar_character(c("a", "b"), "x"),
      "`x` must be length 1"
    )
    
    testthat::expect_error(
      .scholidonline_as_scalar_character(1, "x"),
