...
The function `id_exists()` performs registry-backed validation.
...
Importantly, `NA` does not mean “does not exist”.
It means “cannot be evaluated”.
...
`id_convert()` performs registry-backed crosswalks.
...
`id_metadata()` retrieves structured metadata from registries.
...
`id_links()` returns related identifiers discovered via registry queries.
...
If `provider = "auto"` (default), `scholidonline` selects a
sensible default and may fall back to alternative providers.
...
Layer 1: Syntax and canonical form (`scholid`)
Layer 2: External registry interaction (`scholidonline`)

This separation keeps both packages small, predictable, and maintainable.
...
`scholidonline` checks whether they exist, resolve, and connect
within the scholarly registry ecosystem.
